...
const processParsedData = (data: string[][]) => {
  ...
  const headers = data[headerRowIdx].map(c => String(c || '').toLowerCase().trim());
  const hasHeaderRow = headers.some(h => ['cia', 'voo', 'destino', 'vôo', 'etd', 'eta', 'hora', 'tempo'].includes(h));

  let idxCia = 0, idxVoo = 1, idxDest = 2, idxEtd = 3, idxPref = 4, idxMod = 5, idxEta = 6, idxPos = 7, idxCalco = 8;
  
  if (hasHeaderRow) {
    const getIdx = (keywords: string[]) => headers.findIndex(h => keywords.some(k => h.includes(k)));
    idxCia = getIdx(['cia', 'airline', 'empresa', 'companhia']);
    idxVoo = getIdx(['voo', 'vôo', 'flight']);
    idxDest = getIdx(['destino', 'dest']);
    idxEtd = getIdx(['etd', 'partida', 'saida', 'saída']);
    idxPref = getIdx(['prefixo', 'matrícula', 'matricula', 'reg']);
    idxMod = getIdx(['modelo', 'equipamento', 'eqp', 'tipo']);
    idxEta = getIdx(['eta', 'chegada', 'prev', 'pouso']);
    idxPos = getIdx(['posição', 'posicao', 'pos', 'box']);
    idxCalco = getIdx(['calço', 'calco', 'atual', 'real']);
    
    if (idxVoo === -1 && idxCia === -1) {
       idxVoo = 0;
    }
  }

  const result: MeshFlight[] = [];
  const startRow = hasHeaderRow ? headerRowIdx + 1 : 0;
  
  for (let i = startRow; i < data.length; i++) {
    const row = data[i];
    if (!row || row.length < 2) continue;
    
    const getCol = (idx: number) => (idx >= 0 && idx < row.length) ? String(row[idx] || '').trim() : '';
    ...
    if (hasHeaderRow) {
       ...
       if (idxCia === -1 && voo) {
          const match = voo.match(/^([A-Za-z]{2,3})\s*(\d+)/);
          if (match) {
             cia = match[1].toUpperCase();
          } else {
             cia = 'G3';
          }
       }
       ...
       const col0 = getCol(0);
       const hasNumbers = /\d/.test(col0);
       if (hasNumbers) {
          voo = col0;
          const match = voo.match(/^([A-Za-z]{2,3})/);
          cia = match ? match[1].toUpperCase() : 'G3';
          idxDest = 1; idxEtd = 3; idxEta = 2;
       } else {
          cia = col0;
          voo = getCol(1);
       }
    ...
    result.push({
      id: `imp-${Date.now()}-${i}`,
      airline: cia || 'G3',
      airlineCode: (cia || 'GOL').substring(0,3),
      departureFlightNumber: voo,
      destination: getCol(idxDest >= 0 ? idxDest : 2),
      etd: formatImportTime(getCol(idxEtd >= 0 ? idxEtd : 3)),
      registration: getCol(idxPref >= 0 ? idxPref : 4),
      model: getCol(idxMod >= 0 ? idxMod : 5),
      eta: formatImportTime(getCol(idxEta >= 0 ? idxEta : 6)),
      positionId: getCol(idxPos >= 0 ? idxPos : 7),
      actualArrivalTime: formatImportTime(getCol(idxCalco >= 0 ? idxCalco : 8)),
      isNew: true
    });
  }
  return result;
...
const formatImportTime = (val: string) => {
  ...
  // Excel time is a fraction of a day (e.g. 0.3402777777777778)
  const numVal = Number(strVal.replace(',', '.'));
  if (!isNaN(numVal) && numVal > 0 && numVal <= 1 && (strVal.includes('.') || strVal.includes(','))) {
...
export const OperationalMesh: React.FC<OperationalMeshProps> = ({ onClose, onActivateMesh, isDarkMode, meshFlights, setMeshFlights, setFlights, globalFlights }) => {
  ...
  const filteredFlights = meshFlights.filter(f => {
    ...
    // Check for format errors or garbage
    const garbageWords = ['ench', 'enchimento', 'pre', 'estimado', 'voo', 'vôo', 'destino'];
    const checkGarbage = (f: MeshFlight) => garbageWords.some(w => 
      f.airline.toLowerCase().includes(w) || 
      f.departureFlightNumber.toLowerCase().includes(w) || 
      f.destination.toLowerCase().includes(w)
    );
    const aHasGarbage = checkGarbage(a);
    const bHasGarbage = checkGarbage(b);
    const aHasError = a.etd === '?' || a.eta === '?' || a.actualArrivalTime === '?' || aHasGarbage;
    const bHasError = b.etd === '?' || b.eta === '?' || b.actualArrivalTime === '?' || bHasGarbage;
                        ...
                        const processFile = async (file: File) => {
                          if (file.name.endsWith('.csv')) {
                            ...
                            reader.onload = (evt) => {
                               ...
                               const data = lines.map(line => line.split(new RegExp('[;,]')).map(c => c.trim().replace(new RegExp('^["\']|["\']$', 'g'), '')));
                               handleNewFlights(processParsedData(data));
                            }
          ...
          <table ref={tableRef} className="w-full border-collapse table-fixed select-none min-w-[800px]">
            <thead>
              ...
              {filteredFlights.map((flight, rIdx) => {
                ...
                const garbageWords = ['ench', 'enchimento', 'pre', 'estimado', 'voo', 'vôo', 'destino'];
                const hasGarbage = garbageWords.some(w => 
                  flight.airline.toLowerCase().includes(w) || 
                  flight.departureFlightNumber.toLowerCase().includes(w) ||
                  flight.destination.toLowerCase().includes(w)
                );
                const hasFormatError = flight.etd === '?' || flight.eta === '?' || flight.actualArrivalTime === '?' || hasGarbage;
                ...
                {hasFormatError && (
                  <tr className="bg-orange-500/10 border-b border-orange-500/20">
                    <td colSpan={COLUMNS.length} className="px-3 py-1 text-[10px] text-orange-500 font-bold uppercase tracking-widest text-center">
                      <div className="flex items-center justify-center gap-2">
                        <AlertCircle size={12} />
                        {hasGarbage ? 'Atenção: A linha contém dados incorretos ou não condizentes com a coluna. Verifique ou exclua.' : 'Atenção: Erro de formatação de hora. Corrija as células com "?" antes de enviar.'}
                      </div>
                    </td>
                  </tr>
                )}
