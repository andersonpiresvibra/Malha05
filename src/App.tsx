...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fleetData = await api.getFleet();
        if (fleetData && fleetData.length > 0) {
          const apiVehicles: Vehicle[] = fleetData.map((f: any) => ({
            id: f.id,
            name: f.name,
            type: f.type as 'SRV' | 'CTA',
            status: f.status as any || 'AVAILABLE',
            capacity: f.capacity || 0,
            currentVolume: f.capacity || 0, // Inicia cheio ou 0? Assume cheio.
            operatorId: undefined,
            flightId: undefined
          }));
          setGlobalVehicles(apiVehicles);
        }
      } catch (err) {
        console.error("Falha ao carregar frota da API D1. Usando Mocks...", err);
      }
    };
    fetchData();
  }, []);
